...
const ManageOffers = () => {
  ...
  const handleDelete = async (serviceId, serviceTitle) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          await deleteDoc(doc(db, 'packages', serviceId));
          Swal.fire(
            'Deleted!',
  ...
  return (
    <PageContainer>
      <HeaderContainer>
        <Title>{showForm ? (editingService ? 'Edit Package' : 'Create New Package') : 'Manage Packages'}</Title>
        {!showForm && (
      ...
      {showForm ? (
        <OfferForm initialData={editingService} onSave={handleFormClose} onCancel={handleFormClose} />
      ) : (
        <ServiceGrid>
          {services.length > 0 ? (
            services.map((service) => (
              <ServiceCard key={service.id}>
                ...
                <h4 style={{color:"#222"}}>{service.description}</h4>
                {/* <SectionImage>
            ...
            <NoServicesMessage>No services found. Click "Add New Package" to create one!</NoServicesMessage>
          )}
        </ServiceGrid>
      )}
    </PageContainer>
